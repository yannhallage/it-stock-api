...
export class RepairInterventionPdfDataService {
  buildRepairInterventionSheet(payload: RepairInterventionPrintPayload): RepairInterventionPrintView {
    ...
    return {
      ...
      asset: {
        inventoryNumber: repair.asset.inventoryNumber,
        serialNumber: repair.asset.serialNumber ?? 'N/A',
        type: repair.asset.materialType.name,
        brandModel: `${repair.asset.brand.name} / ${repair.asset.model}`,
        currentStatus: repair.asset.status,
      },
      incident: {
        reference: repair.incident ? `INC-${repair.incident.id}` : 'N/A',
        reportedAt: formatDateTime(repair.incident?.reportedAt),
        department: repair.incident?.department.name ?? 'N/A',
        description: repair.incident?.description ?? 'N/A',
      },
      workshop: {
